...
def BookObjectSelectionHistograms():
    h_dict = {
...
def DrawObjectSelectionHistograms(hs, output_dir = ".", format="png", cH = 800, cW = 700):
    ...
    yMax = max(
        hs["h_nL_raw"].GetMaximum(), 
        hs["h_nL_baseline"].GetMaximum(), 
        hs["h_nL_pass_isolation"].GetMaximum(), 
        hs["h_nL_pass_em_olr"].GetMaximum(), 
        hs["h_nL_pass_fj_olr"].GetMaximum(), 
        hs["h_nL_pass_pTCut"].GetMaximum()
    )
    if hs["h_nL_raw"].Integral() > 0:
        hs["h_nL_raw"].SetMaximum(yMax * 1e5)
        hs["h_nL_raw"].SetLineColor(ROOT.kRed)
        hs["h_nL_raw"].SetLineWidth(3)
        hs["h_nL_raw"].SetLineStyle(2)
        hs["h_nL_raw"].Draw("HIST")
        legend1.AddEntry(hs["h_nL_raw"], "Raw", "l")
    if hs["h_nL_baseline"].Integral() > 0:
        hs["h_nL_baseline"].SetMaximum(yMax * 1e5)
        hs["h_nL_baseline"].SetLineColor(ROOT.kBlack)
        hs["h_nL_baseline"].SetLineWidth(2)
        hs["h_nL_baseline"].SetLineStyle(1)
        hs["h_nL_baseline"].Draw("HIST SAME")
        legend1.AddEntry(hs["h_nL_baseline"], "Baseline", "l")
    if hs["h_nL_pass_isolation"].Integral() > 0:
        hs["h_nL_pass_isolation"].SetMaximum(yMax * 1e5)
        hs["h_nL_pass_isolation"].SetLineColor(ROOT.kBlue)
        hs["h_nL_pass_isolation"].SetLineWidth(2)
        hs["h_nL_pass_isolation"].SetLineStyle(1)
        hs["h_nL_pass_isolation"].Draw("HIST SAME")
        legend1.AddEntry(hs["h_nL_pass_isolation"], "Pass Isolation", "l")
    if hs["h_nL_pass_em_olr"].Integral() > 0:
        hs["h_nL_pass_em_olr"].SetMaximum(yMax * 1e5)
        hs["h_nL_pass_em_olr"].SetLineColor(ROOT.kCyan+1)
        hs["h_nL_pass_em_olr"].SetLineWidth(2)
        hs["h_nL_pass_em_olr"].SetLineStyle(1)
        hs["h_nL_pass_em_olr"].Draw("HIST SAME")
        legend1.AddEntry(hs["h_nL_pass_em_olr"], "Pass e-#mu OLR", "l")
    if hs["h_nL_pass_fj_olr"].Integral() > 0:
        hs["h_nL_pass_fj_olr"].SetMaximum(yMax * 1e5)
        hs["h_nL_pass_fj_olr"].SetLineColor(ROOT.kOrange+1)
        hs["h_nL_pass_fj_olr"].SetLineWidth(2)
        hs["h_nL_pass_fj_olr"].SetLineStyle(1)
        hs["h_nL_pass_fj_olr"].Draw("HIST SAME")
        legend1.AddEntry(hs["h_nL_pass_fj_olr"], "Pass Lep-FatJet OLR", "l")
    if hs["h_nL_pass_pTCut"].Integral() > 0:
        hs["h_nL_pass_pTCut"].SetMaximum(yMax * 1e5)
        hs["h_nL_pass_pTCut"].SetLineColor(ROOT.kMagenta+1)
        hs["h_nL_pass_pTCut"].SetLineWidth(3)
        hs["h_nL_pass_pTCut"].SetLineStyle(1)
        hs["h_nL_pass_pTCut"].Draw("HIST SAME")
        legend1.AddEntry(hs["h_nL_pass_pTCut"], "Pass p_{T} Cut", "l")
    legend1.Draw()
    c1.SaveAs(os.path.join(output_dir, f"lepton_multiplicity.{format}"))
    c1.Close()

    # Ak4 jet multiplicty
    c2 = ROOT.TCanvas("c2", "Jet Multiplicity", cH, cW)
    c2.SetLogy()
    yMax = max(yMax, hs["h_nj_raw"].GetMaximum())
    hs["h_nj_raw"].SetMaximum(yMax * 1e3)
    hs["h_nj_raw"].SetLineColor(ROOT.kRed)
    hs["h_nj_raw"].SetLineWidth(3)
    hs["h_nj_raw"].Draw("HIST")
    c2.SaveAs(os.path.join(output_dir, f"jet_multiplicity.{format}"))
    c2.Close()
    ...
    legend1.SetTextSize(0.035)
    yMax = max(
        hs["h_nJ_raw"].GetMaximum(), 
        hs["h_nJ_baseline"].GetMaximum(), 
        hs["h_nJ_pass_massCut"].GetMaximum(), 
        hs["h_nJ_pass_tau21Cut"].GetMaximum(), 
        hs["h_nJ_pass_pTCut"].GetMaximum()
    )
    hs["h_nJ_raw"].SetMaximum(yMax * 1e5)
    hs["h_nJ_raw"].SetLineColor(ROOT.kRed)
    hs["h_nJ_raw"].SetLineWidth(3)
    hs["h_nJ_raw"].SetLineStyle(2)
    hs["h_nJ_raw"].Draw("HIST")
    legend1.AddEntry(hs["h_nJ_raw"], "Raw", "l")
    hs["h_nJ_baseline"].SetMaximum(yMax * 1e5)
    hs["h_nJ_baseline"].SetLineColor(ROOT.kBlack)
    hs["h_nJ_baseline"].SetLineWidth(2)
    hs["h_nJ_baseline"].SetLineStyle(1)
    hs["h_nJ_baseline"].Draw("HIST SAME")
    legend1.AddEntry(hs["h_nJ_baseline"], "Baseline", "l")
    hs["h_nJ_pass_massCut"].SetMaximum(yMax * 1e5)
    hs["h_nJ_pass_massCut"].SetLineColor(ROOT.kMagenta)
    hs["h_nJ_pass_massCut"].SetLineWidth(2)
    hs["h_nJ_pass_massCut"].SetLineStyle(1)
    hs["h_nJ_pass_massCut"].Draw("HIST SAME")
    legend1.AddEntry(hs["h_nJ_pass_massCut"], "Pass Mass Cut", "l")
    hs["h_nJ_pass_tau21Cut"].SetMaximum(yMax * 1e5)
    hs["h_nJ_pass_tau21Cut"].SetLineColor(ROOT.kBlue)
    hs["h_nJ_pass_tau21Cut"].SetLineWidth(2)
    hs["h_nJ_pass_tau21Cut"].SetLineStyle(1)
    hs["h_nJ_pass_tau21Cut"].Draw("HIST SAME")
    legend1.AddEntry(hs["h_nJ_pass_tau21Cut"], "Pass #tau_{21} Cut", "l")
    hs["h_nJ_pass_pTCut"].SetMaximum(yMax * 1e5)
    hs["h_nJ_pass_pTCut"].SetLineColor(ROOT.kGreen+1)
    hs["h_nJ_pass_pTCut"].SetLineWidth(3)
    hs["h_nJ_pass_pTCut"].SetLineStyle(1)
    hs["h_nJ_pass_pTCut"].Draw("HIST SAME")
    legend1.AddEntry(hs["h_nJ_pass_pTCut"], "Pass p_{T} Cut", "l")
    legend1.Draw()
    c3.SaveAs(os.path.join(output_dir, f"fatjet_multiplicity.{format}"))
    c3.Close()
...
def select_objects(Muon_branch, Electron_branch, FatJet_branch, Jet_branch, objSel_cutflow = {}, objSel_h = None, event_weight = 1.0):
    ...
    if objSel_h != None:
        raw_nL = Muon_branch.GetEntries() + Electron_branch.GetEntries()
        raw_nJ = FatJet_branch.GetEntries()
        raw_nj = Jet_branch.GetEntries()
        objSel_h["h_nL_raw"].Fill(raw_nL, event_weight)
        objSel_h["h_nJ_raw"].Fill(raw_nJ, event_weight)
        objSel_h["h_nj_raw"].Fill(raw_nj, event_weight)

    objSel_cutflow["lepton"]["initial"] += 1
    objSel_cutflow["fatjet"]["initial"] += 1
    ...
    if objSel_h != None:
        objSel_h["h_nL_baseline"].Fill(len(leps), event_weight)
    if len(leps) > 0: 
        objSel_cutflow["lepton"]["baseline"] = objSel_cutflow["lepton"].get("baseline", 0) + 1
    ...
    lep_stages.append(leps)
    if objSel_h != None:
        objSel_h["h_nL_pass_isolation"].Fill(len(leps), event_weight)
    if len(leps) > 0: 
        objSel_cutflow["lepton"]["pass_isolation"] = objSel_cutflow["lepton"].get("pass_isolation", 0) + 1
    ...
    lep_stages.append(leps)
    if objSel_h != None:
        objSel_h["h_nL_pass_em_olr"].Fill(len(leps), event_weight)
    if len(leps) > 0: 
        objSel_cutflow["lepton"]["pass_em_olr"] = objSel_cutflow["lepton"].get("pass_em_olr", 0) + 1
    ...
    if objSel_h != None:
        objSel_h["h_nJ_baseline"].Fill(len(fjs), event_weight)
    if len(fjs) > 0: 
        objSel_cutflow["fatjet"]["baseline"] = objSel_cutflow["fatjet"].get("baseline", 0) + 1
    ...
    fj_stages.append(fjs)
    if objSel_h != None:
        objSel_h["h_nJ_pass_massCut"].Fill(len(fjs), event_weight)
    if len(fjs) > 0: 
        objSel_cutflow["fatjet"]["pass_massCut"] = objSel_cutflow["fatjet"].get("pass_massCut", 0) + 1
    ...
    fj_stages.append(fjs)
    if objSel_h != None:
        objSel_h["h_nJ_pass_tau21Cut"].Fill(len(fjs), event_weight)
    if len(fjs) > 0: 
        objSel_cutflow["fatjet"]["pass_tau21Cut"] = objSel_cutflow["fatjet"].get("pass_tau21Cut", 0) + 1
    ...
    lep_stages.append(leps)
    if objSel_h != None:
        objSel_h["h_nL_pass_fj_olr"].Fill(len(leps), event_weight)
    if len(leps) > 0: 
        objSel_cutflow["lepton"]["pass_fj_olr"] = objSel_cutflow["lepton"].get("pass_fj_olr", 0) + 1
    ...
    lep_stages.append(leps)
    if objSel_h != None:
        objSel_h["h_nL_pass_pTCut"].Fill(len(leps), event_weight)
    if len(leps) > 0: 
        objSel_cutflow["lepton"]["pass_pTCut"] = objSel_cutflow["lepton"].get("pass_pTCut", 0) + 1
    ...
    fj_stages.append(fjs)
    if objSel_h != None:
        objSel_h["h_nJ_pass_pTCut"].Fill(len(fjs), event_weight)
    if len(fjs) > 0: 
        objSel_cutflow["fatjet"]["pass_pTCut"] = objSel_cutflow["fatjet"].get("pass_pTCut", 0) + 1
